...
const Agendadas: React.FC = () => {
  const [agendadas, setAgendadas] = useState<Provider[]>([])
...
const navigateBack = useCallback(() => {
  ...
  return (
    <Container>
      ...
      <ProviderName>Selecione a categoria: {nomeFilial}</ProviderName>
      <ContainerList>
      <Picker
        ...
        <Picker.Item label="Categoria C" value="C" />
        <Picker.Item label="Categoria D" value="D" />
        <Picker.Item label="Categoria E" value="E" />
      </Picker>
      </ContainerList>

      <ButtonBuscaAgendadas onPress={buscarAgendadas}>
        <ButtonBuscaTextAgendadas>Buscar Aulas Agendadas</ButtonBuscaTextAgendadas>
      </ButtonBuscaAgendadas>

      <ProvidersList
        data={agendadas}
        keyExtractor={(agendadas,index) => index.toString()}
        ListHeaderComponent={
          <ProvidersListTitle>Aulas: </ProvidersListTitle>
        }
        renderItem={({ item }) => (
          <ProviderContainer
          ...
          >
            {/* <ProviderAvatar
              source={{
                uri:'https://w7.pngwing.com/pngs/64/222/png-transparent-car-transport-cartoon-car-child-vehicle-cartoon-car.png',
              }}
            /> */}
            ...
            <ProviderInfo>
              ...
              <ProviderMeta>
                ...
                <ProviderMetaText>Hora: {format(new Date([item.AulaDataHora]),'hh:mm:ss')}</ProviderMetaText>
              </ProviderMeta>
              <ProviderMeta>
                <Icon name="user" size={14} color="#ff9000" />
                <ProviderMetaText>Instrutor: {item.InstrutorNome}</ProviderMetaText>
              </ProviderMeta>
              <ProviderMeta>
